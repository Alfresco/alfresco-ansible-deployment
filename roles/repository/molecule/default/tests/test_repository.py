...
def test_alfresco_context_200(host, AnsibleVars):
    ...
    cmd = host.run("curl -iL --user --connect-timeout 5 admin:admin http://{}:8080/alfresco".format(AnsibleVars["repo_host"]))
...
def test_alfresco_api(host, AnsibleVars):
    ...
    cmd = host.run("curl -iL --user --connect-timeout 5 admin:admin http://{}:8080/alfresco/api/discovery".format(AnsibleVars["repo_host"]))
