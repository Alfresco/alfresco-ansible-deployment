...
if [ -n "$MOLECULE_IT_SCENARIO" ]; then
    ...
    if [ "$1" == 'destroy' ]; then
       ...
       pipenv run molecule test && molecule $EXTRA_CONFIG destroy -s "$MOLECULE_IT_SCENARIO"
    elif [ "$1" == 'verify' ]; then
        # shellcheck disable=SC2086
       pipenv run molecule test && molecule $EXTRA_CONFIG converge -s "$MOLECULE_IT_SCENARIO"
        # shellcheck disable=SC2086
        pipenv run molecule test && molecule $EXTRA_CONFIG side-effect -s "$MOLECULE_IT_SCENARIO"
        # shellcheck disable=SC2086
       pipenv run molecule test && molecule $EXTRA_CONFIG verify -s "$MOLECULE_IT_SCENARIO"
    else
