...
def test_share_is_accesible(host, AnsibleVars):
    ...
    while not command or time.time() < timeout:
        run_command = host.run("curl -v -k --connect-timeout 5 --location --request GET 'http://{}/share/page/'".format(AnsibleVars['nginx_1']['ansible_host']))
...
def test_repository_root_is_accesible(host, AnsibleVars):
    ...
    command = False
    while not command or time.time() < timeout:
        run_command = host.run("curl -v -k --connect-timeout 5 --location --request GET 'http://{}/'".format(AnsibleVars['nginx_1']['ansible_host']))
        command = run_command.succeeded
        output = run_command.stdout
...
def test_transformation_stats_is_accesible(host, AnsibleVars):
    ...
    command = False
    while not command or time.time() < timeout:
        run_command = host.run("curl -v -k --connect-timeout 5 http://{}/aio/".format(AnsibleVars['nginx_1']['ansible_host']))
        command = run_command.succeeded
        output = run_command.stdout
...
def test_adw_is_accesible(host, AnsibleVars):
    ...
    command = False
    while not command or time.time() < timeout:
        run_command = host.run("curl -v -k --connect-timeout 5 http://{}/workspace/".format(AnsibleVars['adw_1']['ansible_host']))
        command = run_command.succeeded
        output = run_command.stdout
...
def test_solr_stats_is_accesible(host, AnsibleVars):
    ...
    command = False
    while not command or time.time() < timeout:
        run_command = host.run("curl -v -k --connect-timeout 5 --user admin:admin http://{}/alfresco/s/api/solrstats".format(AnsibleVars['nginx_1']['ansible_host']))
        command = run_command.succeeded
        output = run_command.stdout
